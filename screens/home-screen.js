  ...
  if (!transLations) {
    ...
    return (
      <View style={{flex: 1,alignItems: 'center'}}>
        <Text style={{fontSize:25,color: '#1ebba3',fontWeight: 'bold'}}>{transLations[screenId].mainTitle}</Text>
        <FlatList
        data={DATA}
        renderItem={renderItem}
        keyExtractor={item => item.id}
      />
        <TouchableOpacity style={{backgroundColor:'#FE3C01',padding:10,borderRadius:10,marginTop:5}} onPress={ async()=>{
            await logoutUser();
            await setLogin();
        }}><Text style={{fontSize:20,color: 'white'}}>{transLations[screenId].logout}</Text></TouchableOpacity>
      </View>
    );
  }
}
